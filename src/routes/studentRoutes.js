...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (busId) {
      const bus = await prisma.bus.findUnique({ where: { id: busId } });
      if (!bus) return res.status(400).json({ status: "error", message: "Invalid busId" });
      if (schoolId && bus.schoolId !== schoolId) return res.status(400).json({ status: "error", message: "Bus does not belong to this school" });
    }
    ...
    if (parentId) {
      const parent = await prisma.parent.findUnique({ where: { id: parentId }, include: { user: true } });
      if (!parent) return res.status(400).json({ status: "error", message: "Invalid parentId" });
      if (schoolId && parent.user.schoolId !== schoolId) return res.status(400).json({ status: "error", message: "Parent does not belong to this school" });
    }
