...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // ── Wallet deduction on boarding ─────────────────────────────────
      // Deduct daily subscription fee from parent wallet when child boards (CHECKED_IN).
      // This mirrors checkParentSubscription but fires on the assistant's action,
      // ensuring wallet deducts even before parent opens the tracking screen.
      if (statusEnum === "CHECKED_IN" && studentIdNum) {
        try {
          const studentForWallet = await prisma.student.findFirst({
            where: { id: studentIdNum },
            include: { parent: true },
          });
          if (studentForWallet?.parent) {
            const parentId  = studentForWallet.parent.id;
            const DAILY_FEE = Number(process.env.DAILY_SUBSCRIPTION_FEE) || 10;
            const activeSub = await prisma.subscription.findFirst({
              where: { parentId, studentId: studentIdNum, status: "ACTIVE", expiryDate: { gt: new Date() } },
            });
            if (!activeSub) {
              await prisma.$transaction(async (tx) => {
                const wallet = await tx.wallet.findFirst({ where: { parentId } });
                if (!wallet || wallet.balance < DAILY_FEE) return;
                await tx.wallet.update({ where: { id: wallet.id }, data: { balance: { decrement: DAILY_FEE } } });
                await tx.transaction.create({
                  data: { walletId: wallet.id, parentId, amount: DAILY_FEE, type: "DEDUCTION", status: "SUCCESS", reference: `board-${studentIdNum}-${Date.now()}` },
                });
                const exp = new Date(Date.now() + 24 * 60 * 60 * 1000);
                const existSub = await tx.subscription.findFirst({ where: { parentId, studentId: studentIdNum } });
                if (existSub) {
                  await tx.subscription.update({ where: { id: existSub.id }, data: { status: "ACTIVE", expiryDate: exp, type: "DAILY" } });
                } else {
                  await tx.subscription.create({ data: { parentId, studentId: studentIdNum, status: "ACTIVE", expiryDate: exp, type: "DAILY" } });
                }
              });
            }
          }
        } catch (walletErr) {
          console.warn("[manifest] Wallet deduction on boarding skipped:", walletErr.message);
        }
      }
      ...
      // ⚠️ REMOVED: duplicate notifyParent call was causing double SMS.
      // notifyRecipient above already handles KID mode with the correct message.
